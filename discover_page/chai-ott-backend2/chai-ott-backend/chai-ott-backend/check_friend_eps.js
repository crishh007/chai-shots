...
.then(async () => {
    const eps = await Episode.find({ seriesId: "69e266aedfeb5f4996cbdd86" }).sort({ createdAt: 1 });
    console.log(`Found ${eps.length} episodes for friendship.`);
    eps.forEach(e => console.log(`- ${e.title}: ${e.videoUrl} (Created: ${e.createdAt})`));
    process.exit(0);
})
.catch(err => {
    console.error(err);
