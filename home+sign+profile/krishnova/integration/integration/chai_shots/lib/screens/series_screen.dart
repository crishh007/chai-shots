...
class _SeriesScreenState extends State<SeriesScreen> {
  ...
  Future<void> _loadContent() async {
    List<ContentModel> fetchedEpisodes = [];

    if (widget.initialSeriesId != null && widget.initialSeriesId!.isNotEmpty) {
      try {
        final String url = '${ApiConstants.baseUrl}/episodes/series/${widget.initialSeriesId}';
        final response = await http.get(Uri.parse(url));
        if (response.statusCode == 200) {
          final List<dynamic> data = jsonDecode(response.body);
          fetchedEpisodes = data.map((json) => ContentModel.fromJson(json)).toList();
        }
      } catch (e) {
        debugPrint("Error fetching episodes from backend: $e");
      }
    }
    ...
    if (fetchedEpisodes.isEmpty) {
      ...
      fetchedEpisodes = videoPaths.asMap().entries.map((entry) {
        int idx = entry.key + 1;
...
class _VideoCardSeriesState extends State<VideoCardSeries> {
  ...
  void initState() {
    ...
    if (widget.content.assetPath.startsWith('http://') || widget.content.assetPath.startsWith('https://')) {
