...
class _VideoPlayerPageState extends State<VideoPlayerPage> {
  ...
  Future<void> _loadContent() async {
    ...
    if (mounted) {
      setState(() {
        final String forcedAsset = LocalVideoMapper.forceLocalAsset(widget.videoUrl, widget.videoId);

        // Build base list from local assets
        ...
        // 🔍 IMPORTANT: If the passed videoUrl is a NETWORK URL, it won't be in videoPaths.
        // We must manually add it if it's missing from contentList.
        bool found = false;
        int initialIndex = 0;
        ...
        if (!found && widget.videoUrl.isNotEmpty) {
          // Add the missing network/uploaded video to the start
          final networkContent = ContentModel(
            id: widget.videoId.isNotEmpty ? widget.videoId : "current",
            title: widget.title != 'Title' ? widget.title : "Trending Video",
            rating: "U/A 16+",
            genre: "Trending",
            type: "Short",
            description: "Uploaded content from Chai Shots.",
            assetPath: widget.videoUrl,
            thumbnail: (widget.thumbnailUrl != null && widget.thumbnailUrl!.isNotEmpty) 
          ? widget.thumbnailUrl! 
          : 'https://picsum.photos/seed/${widget.videoId.isNotEmpty ? widget.videoId : 'thumb'}/300/300',
            bgColor: const Color(0xFF5B4A3F),
          );
          contentList.insert(0, networkContent);
          initialIndex = 0;
        }
...
class _VideoCardScrollingState extends State<VideoCardScrolling> {
  ...
  Future<void> _initializePlayer() async {
    ...
    print("PLAYING VIDEO URL: $videoUrl");
    ...
    try {
      ...
      print("VIDEO PLAYBACK ERROR: $e | URL: $videoUrl");
    }
  }
    ...
    if (widget.currentPageIndex != oldWidget.currentPageIndex) {
      if (widget.myIndex == widget.currentPageIndex) {
        ...
        _controller.play().catchError((e) {
          debugPrint("❌ [VideoPlayer] Play error on scroll for ${widget.content.title}: $e");
        });
        _hidePlayPauseIconDelay();
      } else {
        _controller.pause().catchError((e) {
          debugPrint("❌ [VideoPlayer] Pause error on scroll for ${widget.content.title}: $e");
        });
      }
    }
  }
