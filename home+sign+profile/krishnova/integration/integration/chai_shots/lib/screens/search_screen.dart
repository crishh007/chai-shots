...
class _OttSearchScreenState extends State<OttSearchScreen> {
  ...
  void _playVideo(dynamic item) {
    ...
    String thumbnail = item['thumbnail'] ?? item['thumbnailUrl'] ?? '';
    
    print("🎬 [Search Play] Title: $title | ID: $id");
    print("🎥 [Search Play] URL: $url");
    
    if (url.isEmpty) {
      print("⚠️ [Search Play] Warning: Video URL is empty for $title");
      // Optional: Show a snackbar or just return
      return;
    }
  ...
  Widget _buildSearchResults() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String imgUrl = item['thumbnail'] ?? item['thumbnailUrl'] ?? '';
...
// ... rest of the code ...
  ...
  static Future<List<dynamic>> getEpisodesForSeries(String seriesId) async {
    try {
      final String url = '$baseUrl/episodes/series/$seriesId';
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
