...
class _HomePageState extends State<HomePage> {
  ...
  List<ContentModel> _newReleasesSeries = [];
  ...
  Future<void> _loadTrending() async {
    try {
      final data = await ApiService.fetchTrendingSeries();
      if (mounted) {
        setState(() {
          _trendingSeries = data;
  ...
  Future<void> _loadNewReleases() async {
    try {
      final data = await ApiService.fetchNewReleases();
      if (mounted) {
        setState(() {
          _newReleasesSeries = data;
              ...
              child: Container(
                ...
                child: ClipRRect(
                  ...
                  child: imagePath.trim().isEmpty 
                    ? Image.network(
                        'https://picsum.photos/seed/${imagePath.hashCode}/300/450',
                        fit: BoxFit.cover,
                      )
                    : imagePath.startsWith('http')
                        ? Image.network(
                            ...
                            errorBuilder: (context, error, stack) {
                              ...
                              print("THUMBNAIL URL WAS: $imagePath");
  ...
  Widget _horizontalRow({
    ...
    final int count = items?.length ?? 8;
    return SizedBox(
      height: 224 * scale,
      child: ListView.builder(
        ...
        itemBuilder: (ctx, i) {
          String badge = "";
          if (items != null) {
            badge = items[i].rating; // Or use another field
          } else {
            badge = badges != null && badges.isNotEmpty
          ...
          String videoUrl = "";
          if (items != null) {
            imagePath = items[i].thumbnail;
            videoUrl = items[i].assetPath;
          } else {
          ...
          return Padding(
            ...
            child: _portraitCard(
              ...
              imagePath: imagePath,
              videoUrl: videoUrl,
              videoId: items != null ? items[i].id : null,
              videoTitle: items != null ? items[i].title : null,
              badge: badge,
              showRank: showRank,
              rank: i + 1,
              isVideo: i % 4 == 3 || i % 3 == 2,
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _isLoadingNewReleases
                  ? const Center(child: CircularProgressIndicator())
                  : _newReleasesSeries.isEmpty
                      ? _horizontalRow(
                          badges: const ['5 Free Episodes', '2 Free Episodes', '3 Free Episodes', '4 Free Episodes', '1 Free Episode', '3 Free Episodes'],
                        )
                      : _horizontalRow(
                          showRank: false,
                          items: _newReleasesSeries,
                        ),
              const SizedBox(height: 28),
